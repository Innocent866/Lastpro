...
const registration = async (req,res)=>{
    ...
    if(password !== verifypassword){
        res.status(400).json({success:false,message:'password and verifypassword must be the same'})
        return
    }

    try {
        const user = await USER.create({...req.body})
        res.status(201).json({success:true,message:"registration successfully",user})
    } catch (error) {
        if(error.code === 11000){
...
const login = async(req,res)=>{
    ...
    if(!email || !password){
        res.status(400).json({sucess:false,message:'all fields are required to login'})
        return
    }
    try {
...
const forgotPassword = async (req,res)=>{
    ...
    try {
        ...
        await user.save();
        const resetUrl = `https://lastfrontproject.vercel.app/password/${resetToken}`;
